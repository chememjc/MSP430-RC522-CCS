 ...
 *      Author: mcurtis
 */
/* UCB0 -- Registers of Interest
*UCB0CTL0	Control Reg 0
*UCB0CTL1	Control Reg 1
*UCB0BR0	Baud rate control reg 0
*UCB0BR1	Baud rate control reg 1
*UCB0I2CE	Interrup enable register
*UCB0STAT	Status register
*UCB0RXBUF	Receive buffer
*UCB0TXBUF	Transmit buffer
*UCB0I2COA	Own Address
...
unsigned char Rstart=0;

void i2cinit(unsigned int prescaler){
	unsigned char br[2];
	Rstart=0;  //disable repeated shart
	br[0]=prescaler & 0x00FF;
	br[1]=prescaler>>8 & 0x00FF;
	UCB0CTL1 = UCSWRST;                        // Enable SW reset
	UCB0CTL0 = UCMST + UCMODE_3 + UCSYNC;       // I2C Master, synchronous mode
	UCB0CTL1 = UCSSEL_2 + UCSWRST;              // Use SMCLK, keep SW reset
	UCB0BR0 = br[0];                         // set prescaler
	UCB0BR1 = br[1];
	UCB0I2CSA = 0x01;                  // set slave address
	UCB0CTL1 &= ~UCSWRST;                       // Clear SW reset, resume operation
	UCB0I2CIE = UCNACKIE;					//enable interrupt on nack, send stop
	IE2 |= UCB0TXIE;
	return;
}
...
void i2ctx(unsigned char addr, unsigned char *data, unsigned int len){
	unsigned int i;
	UCB0I2CSA = addr;
	UCB0CTL1 |= UCTR+ UCTXSTT; //transmit mode, and start
	for(i=0; i<len; i++){
		UCB0TXBUF=data[i];
...
void i2crx(unsigned char addr, unsigned char *data, unsigned int len){
	...
	UCB0CTL1 |= UCTXSTT;
	for(i=0; i<len; i++){
		while(UCB0CTL1 & UCTXSTT); //wait for data
		data[len]=UCB0RXBUF;
		if(i==len-1) UCB0CTL1 |= UCTXNACK;
	}
	UCB0CTL1 |= UCTXSTT;
	return;
}
void i2ctxrx(unsigned char addr, unsigned char txdata ,unsigned char *data, unsigned int len){
	...
	UCB0CTL1 |= UCTXSTT;
	while(IFG2&UCB0TXIFG); //wait for start/address to be sent
	UCB0TXBUF=txdata;
...
unsigned char i2cslavecheck(unsigned char addr){
	unsigned char ie2bkup, i2ciebkup, retval;
	i2ciebkup = UCB0I2CIE;                      // restore old UCB0I2CIE
	ie2bkup = IE2;                              // store IE2 register
	__disable_interrupt();
	...
	IE2 = ie2bkup;                              // restore IE2
	UCB0I2CIE = i2ciebkup;                      // restore old UCB0CTL1
	__enable_interrupt();
	return retval;                         // return whether or not
                                              	// a NACK occured
}
